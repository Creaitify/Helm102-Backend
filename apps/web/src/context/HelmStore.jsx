...
export function HelmStoreProvider({ children }) {
  ...
  const [byodSnapshot, setByodSnapshot] = useState(null);
  ...
  const fetchByodData = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/byod/current`);
      if (res.ok) {
        const data = await res.json();
        if (data.active) {
          setByodSnapshot(data);
        } else {
          setByodSnapshot(null);
        }
      }
    } catch (err) {
      console.warn('Could not fetch BYOD snapshot', err);
    }
  }, []);

  const uploadByodFile = useCallback(
    async (fileContent, filename, activate = true) => {
      try {
        const res = await fetch(`${API_BASE}/api/byod/upload`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ file_content: fileContent, filename, activate }),
        });
        if (!res.ok) {
    ...
    [fetchByodData, fetchHealth]
  );

  const ingestByodUrl = useCallback(
    async (url, activate = true) => {
      try {
        const res = await fetch(`${API_BASE}/api/byod/url`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const clearByodData = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/byod/current`, { method: 'DELETE' });
      if (res.ok) {
        setByodSnapshot(null);
        await fetchHealth();
        await fetchSyntheticData();
      ...
      console.warn('Could not clear BYOD data', err);
    }
  }, [fetchHealth, fetchSyntheticData]);
...
export function useHelmStore() {
  ...
  if (!context) {
    return {
      ...
      health: { active_model: 'gemini-3.1-flash', gateway_mode: 'live', data_source: 'synthetic', dry_run: true },
      recentRuns: [],
      ...
      uploadByodFile: async () => {},
      ingestByodUrl: async () => {},
      clearByodData: async () => {},
      generateSyntheticScenario: async () => {},
      updateHealthModel: async () => {},
